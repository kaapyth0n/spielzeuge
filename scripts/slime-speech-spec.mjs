...
try {
 ...
 for(const lang of ['ru','de','en']) {
  await page.evaluate(()=>{const key='spielzeuge.slime-check.v1';const s=JSON.parse(localStorage.getItem(key)||'null');if(s){s.energy=100;s.clean=100;s.joy=100;s.friendship=0;s.friendships={};s.baby=null;s.babies=[];localStorage.setItem(key,JSON.stringify(s))}})
  ...
  await page.evaluate(()=>{const key='spielzeuge.slime-check.v1',s=JSON.parse(localStorage.getItem(key));s.energy=100;s.clean=100;s.joy=100;s.friendship=0;s.friendships={};s.baby=null;s.babies=[];localStorage.setItem(key,JSON.stringify(s))})
