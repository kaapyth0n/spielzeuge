...
## Who stands beside the slime

Each family card has “Stand beside me”; “All together” brings the whole family into a separate foreground row. Large families scroll horizontally instead of covering the main slime or shrinking touch targets. Every baby can still be hugged by its own index. The choice is saved as `companion` (an index or `all`), and older saves default to the first baby. The stretch room stays clear; the selection returns in other rooms. Labels and feedback are translated and narrated.

`node scripts/slime-companions-spec.mjs` checks selection, all eight babies, individual hugs, scrolling, persistence, language changes, mobile layout and the stretch room. Use `SLIME_BASE_URL` for production verification.
